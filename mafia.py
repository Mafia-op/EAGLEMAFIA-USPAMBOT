...
from Config import API_ID, API_HASH, STRING, STRING2, STRING3, STRING4 ,STRING5, STRING6, STRING7, STRING8 ,STRING9, STRING10, S_SUDOS, BIO_MESSAGE
import asyncio
...
loop.run_until_complete(start_eagle())       

async def gifspam(e, S):
    try:
        await e.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=sandy.media.document.id,
                    access_hash=S.media.document.access_hash,
                    file_reference=S.media.document.file_reference,
                ),
                unsave=True,
            )
        )
